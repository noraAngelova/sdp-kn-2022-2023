...
private:
	...
	int insertAt(BinTreePosition<AVLElement<T>> p, T const& x) {
		...
		// Проверяваме дали стойността съвпада с тази, която трябва 
		// да бъде добавена
		if ((*p).getData() == x) {
		...
		if (x < (*p).getData()) {
			...
			// 1. Добавянето може да е било неуспешно и тогава трябва само да върнем резултата
			// 2. Добавяне може да е било успешно и да не е промeнило височината на дървото, балансираността е запазена
			// 3. Добавянето може да е било успешно и да има промяна във височината на дървото -> да се провери балансираността
			if (result == 1) { // Разглежда се вариант 3.
			...
			// 1. Добавянето може да е било неуспешно и тогава трябва само да върнем резултата
			// 2. Добавяне може да е било успешно и да не е проминело височината на дървото, балансираността е запазена
			// 3. Дабавянето може да е било успешно и да има промяна във височината на дървото -> да се провери балансираността
			if (result == 1) { // Разглежда се вариант 3 
		...
		// Ако операцията е успешна и балансът се е получил 0, 
		// няма промяна във височината
		if (result >= 0 && (*p).getBalance() == 0) {
			result = 0;
		}

		// Може да е със стойност 1, 0 или -1
		return result;
	}
