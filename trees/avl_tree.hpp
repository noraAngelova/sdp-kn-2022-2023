...
private:
	...
	int insertAt(BinTreePosition<AVLElement<T>> p, T const& x) {
		...
		if (x < (*p).getData()) {
			...
			// Добавяне може да е било успешно и да не е промeнило височината на дървото, балансираността е запазена
			// Добавянето може да е било успешно и да има промяна във височината на дървото -> да се провери балансираността
			if (result == 1) {
