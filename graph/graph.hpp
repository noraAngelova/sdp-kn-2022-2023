...
private:
    ...
    bool findAndRemoveEdge(T const& u, T const& v, bool shouldRemove = true) {
        ...
        // prevVertexIt съдържа референция към предходния връх 
...
public:
    ...
    void removeVertex(T const& v) {
        for (LinkedListIterator<LinkedList<T>> it = g.begin(); it; ++it) {
            ...
            if (*heirIt == v) {
                // Това е списъкът на наследните на върха, 
                // който трябва да бъде изтрит
            ...
            // !!! Трябва да се изтрият всички ребра към изтрия връх
            if (it) {
                heirIt = (*it).begin();
                while (heirIt && *heirIt != v) {
                    heirIt++;
...
bool findPathDFSrec(int a, int b, IntGraph& graph, IntList& visited, IntList& path) {
    ...
    // Не е намерен път от връх u, той се премахва от пътя
    path.deleteLast();

    // Върхът е бил посетен и път не е намерен, 
    // не се премахва от списъка
    // visited.remove(a);
    return false;
}
